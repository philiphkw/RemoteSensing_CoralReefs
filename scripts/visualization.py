...
def animate_timeseries(monthly_rgb, spatial_labels_monthly, timestamps, 
...
def save_monthly_comparisons(monthly_rgb, spatial_labels_monthly, timestamps, 
                              all_timestamps, output_dir, from_year=2023):
    ...
    for ts in tqdm(post_timestamps, desc="Saving monthly comparisons"):
        spatial_idx = all_timestamps.index(ts)
        ...
        im = axes[1].imshow(np.flipud(spatial_labels_monthly[spatial_idx]),
                            cmap=cmap, vmin=-0.5, vmax=config.GMM_COMPONENTS - 0.5,
                            interpolation='nearest', origin='upper')
        ...
        plt.tight_layout()
        plt.savefig(pjoin(output_dir, f'inspect_{ts_str.replace(" ", "_")}.png'), dpi=150, bbox_inches='tight')
        plt.close(fig)


def plot_cluster_means(gmm, feature_names):
    """Plot GMM cluster means as a heatmap to inspect feature contributions."""
    means_df = pd.DataFrame(gmm.means_, columns=feature_names)
    means_df.index.name = 'Cluster'

    fig, ax = plt.subplots(figsize=(12, 8))
    sns.heatmap(means_df, annot=True, fmt='.2f', cmap='RdBu_r', center=0,
                ax=ax, linewidths=0.5)
    ax.set_title('GMM Cluster Means per Feature (standardized)')
    ax.set_xlabel('Feature')
    ax.set_ylabel('Cluster')
    plt.tight_layout()
    plt.savefig(pjoin(config.FIGURES_DIR, f'cluster_means_{config.RUN_NAME}.png'), dpi=300, bbox_inches='tight')
    plt.show()
    return means_df


def plot_cluster_band_timeseries(cluster_id, raw_bands_to_plot, index_bands_to_plot, 
                                  spatial_labels_monthly, bands_all, timestamps):
    fig, ax1 = plt.subplots(figsize=(14, 6))
    ax2 = ax1.twinx()  # secondary axis for indices
    ...
    ax1.set_xlabel('Date')
    ax1.set_title(f'Cluster {cluster_id} — Mean Band Values Over Time')
    ...
    plt.tight_layout()
    plt.savefig(pjoin(config.FIGURES_DIR, f'cluster_{cluster_id}_timeseries_{config.RUN_NAME}.png'), dpi=300, bbox_inches='tight')
    plt.show()
